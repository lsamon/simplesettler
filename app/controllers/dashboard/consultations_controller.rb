class Dashboard::ConsultationsController < ApplicationController
  ...
  def create
    user_params = params.fetch(:user_detail, {}).permit(:visa_status,:visa_help_type, :is_currently_in_desired_country,:done_ielts,:visa_expiry_date, :country_of_passport, :resume)
    appointment_params = params[:user_detail][:appointment].permit(:appointment_date,:require_translator, :language, :appointment_type)
