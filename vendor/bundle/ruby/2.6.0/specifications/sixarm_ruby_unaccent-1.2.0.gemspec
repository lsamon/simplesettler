# -*- encoding: utf-8 -*-
# stub: sixarm_ruby_unaccent 1.2.0 ruby lib

Gem::Specification.new do |s|
  s.name = "sixarm_ruby_unaccent".freeze
  s.version = "1.2.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["SixArm".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2017-08-13"
  s.description = "Unaccent replaces a string's accented characters with unaccented characters".freeze
  s.email = "sixarm@sixarm.com".freeze
  s.homepage = "http://sixarm.com/".freeze
  s.licenses = ["Apache-2.0".freeze, "Artistic-2.0".freeze, "BSD-3-Clause".freeze, "GPL-3.0".freeze, "MIT".freeze, "MPL-2.0".freeze]
  s.rubygems_version = "3.0.1".freeze
  s.summary = "SixArm.com \u2192 Ruby \u2192 Unaccent".freeze

  s.installed_by_version = "3.0.1" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_development_dependency(%q<minitest>.freeze, [">= 5.7.0", "< 6"])
      s.add_development_dependency(%q<sixarm_ruby_minitest_extensions>.freeze, ["= 1.0.5"])
      s.add_development_dependency(%q<rake>.freeze, ["> 10.4.2", "< 11"])
      s.add_development_dependency(%q<simplecov>.freeze, [">= 0.10.0", "< 2"])
      s.add_development_dependency(%q<coveralls>.freeze, [">= 0.8.2", "< 2"])
    else
      s.add_dependency(%q<minitest>.freeze, [">= 5.7.0", "< 6"])
      s.add_dependency(%q<sixarm_ruby_minitest_extensions>.freeze, ["= 1.0.5"])
      s.add_dependency(%q<rake>.freeze, ["> 10.4.2", "< 11"])
      s.add_dependency(%q<simplecov>.freeze, [">= 0.10.0", "< 2"])
      s.add_dependency(%q<coveralls>.freeze, [">= 0.8.2", "< 2"])
    end
  else
    s.add_dependency(%q<minitest>.freeze, [">= 5.7.0", "< 6"])
    s.add_dependency(%q<sixarm_ruby_minitest_extensions>.freeze, ["= 1.0.5"])
    s.add_dependency(%q<rake>.freeze, ["> 10.4.2", "< 11"])
    s.add_dependency(%q<simplecov>.freeze, [">= 0.10.0", "< 2"])
    s.add_dependency(%q<coveralls>.freeze, [">= 0.8.2", "< 2"])
  end
end
